...
$(document).on('click', '.like-button', function() {
    ...
    const likesCountElement = button.closest('.post-card, .post-detail-card').find('.likes-count');
...
$(document).on('click', '.follow-btn', function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            const followersCountElement = $('.user-stats .count').eq(1);
            if (followersCountElement.length) {
                followersCountElement.text(response.followers_count);
            }
        }
    });
});
